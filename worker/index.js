...
<body data-page="${route}">
  ...
  <script>
    ...
    const publicTile = c =>
      ...
      '<b class="grade">'+safe(c.grade)+'</b>'+(qty(c)>1?'<b class="quantity">×'+qty(c)+'</b>':'')+'</div>'+
    ...
    function renderCollectors(){
      ...
      list.innerHTML=collectorResults.map(p=>
        ...
        '<span><b>'+safe(p.display_name||('@'+p.handle))+'</b>'+
        '<small>@'+safe(p.handle)+(p.cards?' · '+p.cards+' shared card'+(p.cards===1?'':'s'):'')+'</small></span></button>'
      ).join('');
    ...
    async function searchCollectors(term){
      try{
        ...
        // One extra request gives every result a card count, which is the
        // difference between a name and a reason to click it.
        if(collectorResults.length){
          try{
            let handles=collectorResults.map(r=>'"'+r.handle+'"').join(',');
            let cards=await sb('/rest/v1/public_cards?select=handle&limit=1000&handle=in.('+encodeURIComponent(handles)+')');
            let counts={};
            cards.forEach(c=>{counts[c.handle]=(counts[c.handle]||0)+1});
            collectorResults.forEach(r=>{r.cards=counts[r.handle]||0});
          }catch(e){}
        }
        renderCollectors();
      }catch(err){
    ...
    async function loadShowcase(){
      ...
      try{
        ...
        $('#showcaseMeta').textContent=list.length?list.length+' shared card'+(list.length===1?'':'s')+' · @'+SHOWCASE_HANDLE:'';
        const paint=()=>{
