...
<body data-page="${route}">
  ...
  <script>
    ...
    function renderPublicResults(){
      ...
      $('#searchCount').textContent=publicResults.length?publicResults.length+' shared card'+(publicResults.length===1?'':'s'):'';
    ...
    async function runPublicSearch(){
      ...
      $('#publicEmptyText').textContent=term?'Try another player or set.':'Type a player, set or team to begin.';
      if(!term){publicResults=[];renderPublicResults();return}
      try{
        let like='*'+term+'*';
