...
<body data-page="${route}">
  ...
  <script>
    ...
    async function loadCloudCards(){let rows=await sb('/rest/v1/cards?select=*&order=created_at.desc'),loaded=rows.map(rowToCard);for(let c of loaded){c.photo=await signedPhoto(c.frontImagePath);c.photoBack=await signedPhoto(c.backImagePath)}lastPhotoSign=Date.now();customCards=loaded;cards=customCards;render();renderPricing();refreshSets();updateTotals()}
